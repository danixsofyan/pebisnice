...
3. **RLS aktif sejak migrasi pertama v2.0.** Semua tabel bisnis memakai `ENABLE` + `FORCE ROW LEVEL SECURITY` dengan policy `project_id = current_setting('app.current_project_id')::uuid`. DAL membungkus akses dalam transaksi yang menjalankan `SET LOCAL` lewat helper `withTenant()`.
...
- Pagination keyset menggantikan OFFSET pada daftar transaksi dan audit log.
...
Vitest dan Playwright sudah terpasang di `package.json` tetapi belum ada satupun file test — ini diperbaiki di Tahap 1.
...
| Risiko                                       | Mitigasi                                                                        |
| -------------------------------------------- | ------------------------------------------------------------------------------- |
| Migrasi TIMESTAMPTZ pada data yang sudah ada | `USING kolom AT TIME ZONE 'Asia/Jakarta'`; dites di staging dengan salinan data |
